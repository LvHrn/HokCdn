 ...
 * 激活列表特效
 */
function initArchiveListEffect() {
    const wow = $('.hok_archive__list').attr('data-wow');
 ...
 * 初始化统计页面
 */
function initCensusPage() {
    if (document.querySelector('.hok_census__basic') === null) return
    /* 初始化统计 */
    initCensusData()
    initCensusCategory()
    initCommentLately()
...
function initCensusData() {
    ...
    const bytesToSize = bytes => {
        if (!bytes) return '0 B';
        const k = 1000, sizes = ['B', 'KB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB'],
    ...
    function initChart() {
        $.ajax({
            url: Hok.BASE_API, type: 'POST', dataType: 'json', data: {
                routeType: 'server_status'
            }, success(res) {
                if (!res.status) Qmsg.warning('服务器接口异常！');
                {
                    $('.hok_census__server-item .count .core').html(`${res.cpu[1]} 核`);
                    $('.hok_census__server-item .count .ram').html(`${megaknotsToSize(res.memory.memTotal)}`);
                    $('.hok_census__server-item .count .up').html(`总发送：${bytesToSize(res.upTotal)}`);
                    $('.hok_census__server-item .count .down').html(`总接收：${bytesToSize(res.downTotal)}`);
                    ...
                    flowChart.setOption({
                        title: {
                            ...
                            type: 'line', name: '上行', smooth: true, showSymbol: false, itemStyle: {
                                normal: {
                                    color: '#f39494', areaStyle: {
                                        color: '#f39494'
                                    }, lineStyle: {
                                        width: 2, color: '#f39494'
                                    }
                                }
                            }, stack: '总量', data: upSeries
                        }, {
                            type: 'line', name: '下行', smooth: true, showSymbol: false, itemStyle: {
                                normal: {
                                    color: '#9dd3e8', areaStyle: {
                                        color: '#9dd3e8'
                                    }, lineStyle: {
                                        width: 2, color: '#9dd3e8'
                                    }
                                }
                            }, stack: '总量', data: downSeries
                        }]
                    });
                }
                {
                    ...
                    workChart.setOption({
                        title: {
                            ...
                            data: [{
                                name: 'CPU占用', value: cpuUse, itemStyle: {
                                    color: '#b3c25a'
                                }
                            }, {
                                name: '内存占用', value: memoryRealUse, itemStyle: {
                                    color: '#67b580'
                                }
                            }, {
                                name: '系统缓冲', value: memoryBufferUse, itemStyle: {
                                    color: '#86ba71'
                                }
                            }, {
                                name: '内存缓冲', value: memoryCacheUse, itemStyle: {
                                    color: '#feb041'
                                }
                            }, {
                                name: '系统负载', value: systemLoad, itemStyle: {
                                    color: '#fd7e55'
                                }
                            }], type: 'bar', showBackground: true, label: {
                                show: true, color: '#ffffff', formatter: params => `${params.data.value}%`
                            }, backgroundStyle: {
 ...
 * 初始化统计
 */
function initCensusCategory() {
    const categoryDom = document.querySelector('#category');
    const categoryChart = echarts.init(categoryDom);
    const seriesData = [];
    $('.hok_census__basic-item.category ul li').each((index, item) => {
        seriesData.push({
            name: item.getAttribute('data-name'), value: item.getAttribute('data-value')
        });
    });
    categoryChart.setOption({
        tooltip: {
            ...
            type: 'pie', roseType: 'area', itemStyle: {
                borderRadius: 8
            }, data: seriesData
        }]
    });
        ...
        url: Hok.BASE_API, type: 'POST', dataType: 'json', data: {
            ...
            latelyChart.setOption({
                title: {
                    ...
                    name: '数量', itemStyle: {
                        normal: {
                            color: '#91cc75', lineStyle: {
                                width: 2, color: '#91cc75'
                            }
                        }
                    }, data: res.series, type: 'line', smooth: true
                }
            });
        }
    });
...
function initArchive() {
    ...
    function initFiling() {
        if ($('.hok_census__filing .button').html() === 'loading...') return;
        $.ajax({
            url: Hok.BASE_API, type: 'POST', dataType: 'json', data: {
                routeType: 'article_filing', page: ++page
            }, success(res) {
                if (!res.length) {
                    $('.hok_census__filing .item.load').remove();
                ...
                $('.hok_census__filing .button').html('加载更多');
            }
        });
    }

    $('.hok_census__filing .button').on('click', function () {
        initFiling();
        $(this).html('loading...');
    });
}
 ...
 * 控制归档加载更多
 */
$(document).on('click', '.hok_census__filing .content .panel', function () {
...
function initLeavingPage() {
    ...
    const maxWidth = $('.hok_detail__leaving-list').width();
    const maxHeight = $('.hok_detail__leaving-list').height();
    ...
    $('.hok_detail__leaving-list .item').each((index, item) => {
