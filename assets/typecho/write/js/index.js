...
import HokAction from './_actions';
import createPreviewHtml from './_create';

class Hok extends HokAction {
	constructor() {
		super();
				...
				extensions: [
					...
					EditorView.domEventHandlers({
						paste: e => {
							...
							if (!blob) return;
							let api = window.HokConfig.uploadAPI;
							if (!api) return;
						...
						scroll: e => {
							if (!window.HokConfig.canPreview) return;
				...
				el.on('click', e => {
					...
					switch (item.type) {
						...
						case 'about':
							...
							break;
						case 'character':
							super._createTableLists(this.cm, HokConfig.characterAPI, '星星符号', '字符大全');
							break;
						case 'emoji':
							super._createTableLists(this.cm, HokConfig.emojiAPI, '表情', '符号表情（需数据库支持）');
							break;
						case 'task-no':
						...
						case 'preview':
							...
							if (el.hasClass('active')) window.HokConfig.canPreview = true;
							else window.HokConfig.canPreview = false;
							createPreviewHtml(this.cm.state.doc.toString());
							window.HokConfig.canPreview && super._updateScroller(document.querySelector('.cm-scroller'), document.querySelector('.cm-preview'));
							break;
					}
				});
				$('.cm-tools').append(el);
	...
	/* 已测 √ */
	init_AutoSave() {
		if (window.HokConfig.autoSave !== 1) return;
...
document.addEventListener('DOMContentLoaded', () => new Hok());
